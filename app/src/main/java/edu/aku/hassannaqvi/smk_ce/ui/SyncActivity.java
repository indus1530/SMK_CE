...
import edu.aku.hassannaqvi.smk_ce.workers.DataDownWorkerALL;
import edu.aku.hassannaqvi.smk_ce.workers.DataUpWorkerALL;
import edu.aku.hassannaqvi.smk_ce.workers.PhotoUploadWorker2;

import static edu.aku.hassannaqvi.smk_ce.core.MainApp.sdDir;
import static edu.aku.hassannaqvi.smk_ce.database.CreateTable.PROJECT_NAME;
import static edu.aku.hassannaqvi.smk_ce.utils.AndroidUtilityKt.isNetworkConnected;
import static edu.aku.hassannaqvi.smk_ce.utils.AppUtilsKt.dbBackup;
    ...
    public void ProcessStart(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.btnSync:
                ...
                downloadTables.add(new SyncModel(Province.TableProvince.TABLE_NAME));
                /*downloadTables.add(new SyncModel(RsdHF.TablersdHF.TABLE_NAME));*/
                                ...
                                try {
                                    ...
                                    switch (tableName) {
                                        ...
                                        case HealthFacilities.TableHealthFacilities.TABLE_NAME:
                                            ...
                                            Log.d(TAG, "onChanged: " + tableName + " " + workInfo.getOutputData().getInt("position", 0));
                                            break;
                                        case Province.TableProvince.TABLE_NAME:
                                            jsonArray = new JSONArray(result);
                                            insertCount = db.syncProvince(jsonArray);
                                            Log.d(TAG, "onChanged: " + tableName + " " + workInfo.getOutputData().getInt("position", 0));
                                            break;
                                 /*       case RsdHF.TablersdHF.TABLE_NAME:
                                            jsonArray = new JSONArray(result);
                                            insertCount = db.syncRsdHF(jsonArray);
                                            Log.d(TAG, "onChanged: " + tableName + " " + workInfo.getOutputData().getInt("position", 0));
                                            break;*/
